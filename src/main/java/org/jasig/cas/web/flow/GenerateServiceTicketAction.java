...
public final class GenerateServiceTicketAction extends AbstractAction {
    ...
    protected Event doExecute(final RequestContext context) {
        ...
        ParameterMap pm= context.getRequestParameters();
        String username=pm.get("username");
        try {
        	if (null!=username) {
        		User user=new User();
        		user.setLoginName(username);
        		user.setToken(ticketGrantingTicket);
        		redisTemplate.opsForValue().set(username, JSON.toJSON(user), 1800, TimeUnit.SECONDS);
			}
